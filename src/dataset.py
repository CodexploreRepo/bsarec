...
class RecDataset(Dataset):
    def __init__(self, args, user_seq, test_neg_items=None, data_type="train"):
        ...
        self.user_ids = []
        self.contrastive_learning = args.model_type.lower() in [
            "fearec",
            "duorec",
        ]
        ...
        if self.contrastive_learning and self.data_type == "train":
            if os.path.exists(args.same_target_path):
                self.same_target_index = np.load(
                    args.same_target_path, allow_pickle=True
                )
            else:
    ...
    def __getitem__(self, index):
        ...
        if self.data_type in ["valid", "test"]:
            ...
            cur_tensors = (
                torch.tensor(
                    self.user_ids[index], dtype=torch.long
                ),  # user_id for testing
                torch.tensor(input_ids, dtype=torch.long),
                torch.tensor(answer, dtype=torch.long),
...
def get_rating_matrix(data_name, seq_dic, max_item):
    ...
    valid_rating_matrix = generate_rating_matrix_valid(
        seq_dic["user_seq"], seq_dic["num_users"], num_items
    )
